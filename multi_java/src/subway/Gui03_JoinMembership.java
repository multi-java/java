...
public class Gui03_JoinMembership extends JFrame {
	...
	public Gui03_JoinMembership() {
		...
		/**
		 * 4-2) 북쪽의 pN: 회원가입 로고와 뒤로가기 버튼 부착 - 위쪽으로 여백이 생기도록 pN의 레이아웃을 BorderLayout으로 설정
		 */
		...
		/**
		 * 4-3) 중앙의 pC에 회원가입에 대한 정보 입력칸 -라벨과 입력칸을 넣기위해 GridLayout 5,2 설정
		 */
		...
		pC.add(pn1);
		/**
		 * 5줄, 2열의 GridLayout에 넣을 두 번째 패널: 아이디 입력칸 -tId를 GridLayout에 넣지 않고 패널을 통하여 넣으면
		 * 텍스트필드가 패널크기와 같지않으니 보다 얇은 높이로 형성 - 아래의 입력칸들에 같은 효과
		 */
			...
			if (obj == bJoin) {
				...
				for (Customer customer : customers) {
					if (customer.getId().equals(id)) {
						JOptionPane.showMessageDialog(p, "이미 가입한 이이디입니다. 다른 아이디를 사용해주세요.", "회원정보",
